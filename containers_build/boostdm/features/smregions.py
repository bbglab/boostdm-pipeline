...
def motifs(significant_regions):
    ...
    pfam_regions = pd.read_csv(PFAM_DOMAINS_FILE, sep='\t', low_memory=False, names=[
        'CHROMOSOME', 'START', 'STOP', 'STRAND', 'ELEMENT', 'ELEMENT2', 'SYMBOL'
    ])
...
def generate(files, qval_thresh=0.1):
    """group a set of output files into a single dataframe"""
    df = pd.DataFrame()

    for input_file in files:
        df_dom = pd.read_csv(input_file, sep='\t')
        df = df.append(df_dom[(df_dom['Q_VALUE'] < qval_thresh) & ((df_dom['OBSERVED_REGION'] / df_dom['MEAN_SIMULATED']) > 1)])
