...
def curve_fit(func, x_data, y_data, seed_params=None, weights=None, bounds=None):
    """custom curve fit function with a vector of deconstructsigs"""
...
def cli(evaluation_path, mutations, samples, output):
    ...
    for ttype in tqdm.tqdm(gene_ttype_iterable):
        if ttype not in samples_info:
            continue
        cohorts_in_ttype, _ = list(zip(*tree.get_cohorts(ttype)))
        df_observed_ttype = df_mutations[df_mutations['COHORT'].isin(cohorts_in_ttype)]
    ...
    df_discovery_index.to_csv(output, sep='\t', index=False)
