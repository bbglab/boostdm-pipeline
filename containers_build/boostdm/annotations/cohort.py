...
from boostdm.globals import CDS_FILE, COHORTS_PATH, DRIVERS_PATH
...
def oncotree_sisters(cohort):
    ...
    cohorts_list = tree.get_cohorts(parent)
...
def mnvs_to_remove():
    ...
    mnvs_path = PathConfig.mnvs
    mnvs_df = pd.read_csv(mnvs_path, sep='\t')
...
def retrieve_cds():
    ...
    """Returns dataframe with CDS regions"""

    cds_df = pd.read_csv(CDS_FILE, sep='\t', header=None, compression='gzip', low_memory=False, skiprows=1)
...
def build_table(cohort, dndscv_file, dndscv_annotated_file,
                mutrate_fn, clustl_group_file, hotmaps_group_file, smregions_group_file, xs_thresh=0.85, n_splits=50):
...
def cli(cohort, dndscv_path, dnds_muts_path, mutrate_path, clustl_group_path,
    ...
    df = build_table(cohort, dndscv_path, dnds_muts_path, mutrate_path, clustl_group_path, 
                     hotmaps_group_path, smregions_group_path, n_splits=splits, xs_thresh=threshold)
