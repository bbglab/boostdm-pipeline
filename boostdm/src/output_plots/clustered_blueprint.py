...
def create_saturation_vectors():
    ...
    for fn in tqdm.tqdm(
        glob.glob(os.path.join(saturation_folder, "*.prediction.tsv.gz"))
    ):
...
def clustered_blueprints(gene, saturation_vectors, df_pfam, df_names):
    ...
    if X.shape[0] > 20:
        ...
        gs = gridspec.GridSpec(
            figure=fig, ncols=2, nrows=2, width_ratios=[15, 1], height_ratios=[1, 20]
        )
        ...
        fig = plt.figure(figsize=(13, fig_height))
        gs = gridspec.GridSpec(
            figure=fig, ncols=2, nrows=2, width_ratios=[15, 1], height_ratios=[1, 13]
        )
        gs.update(hspace=0.05, wspace=0.00)
        ...
        fig = plt.figure(figsize=(13, fig_height))
        gs = gridspec.GridSpec(
            figure=fig, ncols=2, nrows=2, width_ratios=[15, 1], height_ratios=[1, 8]
        )
        gs.update(hspace=0.05, wspace=0.00)
        ...
        fig = plt.figure(figsize=(13, fig_height))
        gs = gridspec.GridSpec(
            figure=fig, ncols=2, nrows=2, width_ratios=[15, 1], height_ratios=[1, 4]
        )
        gs.update(hspace=0.05, wspace=0.00)
    ...
    if df_pfam[df_pfam["SYMBOL"] == gene].shape[0] > 0:
        ...
        for i, r in dg_pfam.sort_values(by="START").reset_index().iterrows():
            ...
            if gene in [
                ...
                y_min, y_max = -30, 70
                gap = y_max - y_min
                levels = [y_min, y_min + 0.4 * gap, y_min + 0.7 * gap, y_min + 1 * gap]
                ax0.annotate(
                    r["DOMAIN_NAME"],
                    xy=(start_base + 2, levels[i % 4]),
                    fontsize=7,
                    zorder=10,
                    rotation=0,
                )
                ...
                ax0.annotate(
                    r["DOMAIN_NAME"], xy=(start_base + 2, 1), fontsize=7, zorder=10
                )
                ax0.set_ylim(-10, 10)
            else:
                y_min, y_max = -30, 70
    ...
    y_values = np.array(y_values) + np.random.normal(
        0, sigma_jitter, size=len(y_values)
    )

    ax2.scatter(x_values, y_values, color="#cc0000", s=1.0, alpha=0.5)
    ...
    ax2.set_xticklabels(
        [str(int(x)) for x in np.linspace(0, len_aa, num=10, endpoint=True)]
    )
    ...
    if X.shape[0] > 1:
        ...
        h = ax2.hlines(
            np.linspace(min_, stop=max_, num=len(labels)),
            xmin=0,
            xmax=len_aa,
            alpha=0.3,
        )
        ...
        _ = ax2.set_yticklabels(labels, rotation=0, fontsize=7)
        h = ax2.hlines(
            np.linspace(0, stop=len(X) + 1, num=len(labels)),
            xmin=0,
            xmax=len_aa,
            alpha=0.3,
        )
...
def cli():
    ...
    regions_fn = os.path.join(
        os.environ["BOOSTDM_DATASETS"], "saturation", "cds-5spli.regions.gz"
    )

    regions = pd.read_csv(
        regions_fn, sep="\t", usecols=[5, 6], low_memory=False
    ).drop_duplicates()
    df_pfam = pd.read_csv(
        PFAM_file,
        sep="\t",
        names=["ENSEMBL_GENE", "TRANSCRIPT_ID", "START", "END", "DOMAIN"],
    )
    ...
    drivers = pd.read_csv(
        os.path.join(os.environ["INTOGEN_DATASETS"], "drivers.tsv"), sep="\t"
    )
