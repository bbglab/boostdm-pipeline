...
def discovery_run(samples, mutations, iterations=100, ngrid=20):
    ...
    params_list, disc_ind, grid, unique_counts = discovery_index_with_bootstrap(
...
def cli(evaluation_path, mutations, samples, output):
    ...
    for ttype in tqdm.tqdm(gene_ttype_iterable):
        ...
        for gene in gene_ttype_iterable[ttype]:
            ...
            try:
                n_samples, n_unique, discovery, interquartile_range = discovery_run(
                    samples_info[ttype], df_observed
                )
            except Exception:
                # TODO: implement a more consistent logging for failed cases?
                print(
                    f"Discovery index calculation failed for gene {gene}, tumor-type {ttype}"
                )
                continue
