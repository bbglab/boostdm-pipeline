...
cohorts_path = (
    "/workspace/projects/intogen_2017/postprocess/pipeline/20200213/cohorts.tsv"
)
...
def get_sample_info():
    ...
    for d in [intogen_samples, stjude_samples, hartwig_samples]:
        for cohort in d:
            ...
            samples_info[ttype] += (
                list(d[cohort]["samples"]["mut_per_sample"].keys()) + list_hypermutators
            )
    return samples_info
...
def get_number_mutations(evaluation_folder):
    ...
    for fn in tqdm.tqdm(glob.glob(os.path.join(evaluation_folder, "*"))):
        ...
        for gn in list(
            glob.glob(os.path.join(evaluation_folder, f"{ttype}/*.eval.pickle.gz"))
        ):
