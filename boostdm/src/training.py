...
from src.globals import XGB_PARAMS
...
def train(values):
    ...
    params["n_estimators"] = (
        20000  # set it high enough to allow "early stopping" events below
    )
    ...
    learning_curve_dict = {
        k: v["logloss"][: params["n_estimators"]]
        for k, v in learning_curve_dict.items()
    }
...
@click.option(
    "--min-rows",
    type=int,
    help="Minimum number of rows to carry out training",
    default=30,
)
