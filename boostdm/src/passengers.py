...
from src.globals import MANE_TRANSCRIPTS_FILE, GENOME_BUILD
...
contexts_tuple_format = sorted(
    [(s, f) for s, f in product(subs, flanks)], key=lambda x: (x[0], x[1])
)
contexts_mutrate_format = [
    f[0] + s[0] + f[1] + ">" + s[1] for s, f in contexts_tuple_format
]


CB = {"A": "T", "C": "G", "G": "C", "T": "A"}
TRIPLETS = [
    p[0] + c + p[1] for c in ["C", "T"] for p in itertools.product(CB.keys(), repeat=2)
]
CHANGES = {"C": ["A", "G", "T"], "T": ["A", "C", "G"]}

cds_data = pd.read_csv(
    MANE_TRANSCRIPTS_FILE, sep="\t", header=None, compression="gzip", low_memory=False
)
    ...
    for ind, row in df.iterrows():
        exons.append(
            func(
                row["chr"],
                int(row["start"]) - 1,
                size=int(row["end"]) - int(row["start"]) + 3,
            )
        )
...
def randomize(mutrate, chrom, cds, segments, n_randomizations):
    ...
    try:
        random_draw = np.random.choice(
            np.arange(len(prob)), size=n_randomizations, p=prob, replace=True
        )
    except ValueError:
        prob = (
            1 / (len(prob)) * np.ones_like(prob)
        )  # prob: extended sequence with x3 as many positions
