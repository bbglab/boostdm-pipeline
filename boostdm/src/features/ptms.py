...
from src.globals import PTMS_FILE
...
def get_ptms(d_data, hugo, mutation, restrict_missense=False):
    ...
    if (hugo in d_data) and (aa in d_data[hugo]):
        ...
        return data_vector[
            [0, 5, 3, 7, 4]
        ]  # 0: acetylation; 5: methylation; 3: phosphorylation;
...
def add_ptms(df):
    ...
    df[ptms_cols] = df[["gene", "aachange"]].apply(
        lambda r: pd.Series(get_annotation(*r)), axis=1
    )
    return df
