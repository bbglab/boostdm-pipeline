...
from src.globals import PFAM_DOMAINS_FILE
    ...
    for i, row in df.iterrows():
        ...
        if len(motifs_in_trans) > 0:
            for ix, line in motifs_in_trans.iterrows():
                if (
                    (int(line["START"]) < row["pos"])
                    and (row["pos"] < int(line["STOP"]))
                    and (row["csqn_type"] != "synonymous_variant")
                ):
...
def motifs(significant_regions):
    ...
    selected_trans_pfam = pfam_regions[
        pfam_regions["ELEMENT"].isin(significant_regions)
    ]
...
def generate(files_list, thresh=0.1):
    ...
    for input_file in files_list:
        ...
        df.append(
            df_dom[
                (df_dom["Q_VALUE"] < thresh)
                & ((df_dom["OBSERVED_REGION"] / df_dom["MEAN_SIMULATED"]) > 1)
            ]
        )
