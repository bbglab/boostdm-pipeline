...
def assign(chr_, pos, clusters):
    ...
    if len(cluster1) > 0:
        return 2.0  # tumor type specific cluster
        ...
        if len(cluster2) > 0:
            return 1.0  # another tumor type cluster

    return 0.0  # no cluster


def add_feature(df, specific_df, global_df):
    clusters = {"specific": specific_df, "pan": global_df}
...
def generate(files, thresh=0.05):
    ...
    for input_file in files:
        input = pd.read_csv(
            ...
            sep="\t",
            header=0,
            usecols=["chromosome", "genomic position", "q-value"],
        ).drop_duplicates()
        df = pd.concat(
            [df, input[input["q-value"] <= thresh]], ignore_index=True, sort=False
        )
