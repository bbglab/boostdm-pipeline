...
from src.globals import TABIX_FILE
from src.vepreader import Tabix
...
def get_aachange(chr_, pos, alt, gene, reader):
    for data in reader.get(chr_, pos, pos):
        ...
        correct_gene = (
            data["SYMBOL"] == gene
        )  # skip cases with antisense overlapping gene (gene is gene_symbol)
...
def add_feature(df):
    with Tabix(TABIX_FILE) as reader:
        ...
        df["aachange"] = df.apply(
            lambda row: get_from_reader(
                str(row["chr"]), int(row["pos"]), row["alt"], row["gene"]
            ),
            axis=1,
        )
    return df
...
def test():
    """Test function"""
    df = pd.DataFrame(
        {"chr": ["14"], "pos": [104773083], "alt": ["A"], "gene": ["AKT1"]}
    )
    df = add_feature(df)
    print(df)
