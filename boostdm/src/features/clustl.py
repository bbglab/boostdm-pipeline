...
def assign(chr_, pos, clusters):
    ...
    if e1:
        return 2.0  # cluster in specific tumor type
    elif e2:
        return 1.0  # cluster in another tumor type
    else:
...
def generate(files, thresh=0.05):
    ...
    for file in files:
        ...
        input3["5_COORD"], input3["3_COORD"] = zip(
            *input3[0].map(lambda x: tuple(x.split(",")))
        )
        del input3[0]
