...
def attribution_func(d, df, models_path):
    ...
    for gene in d[ttype]:
        ...
        df_slice = df[
            (df["selected_model_ttype"] == ttype) & (df["selected_model_gene"] == gene)
        ].copy()
...
def get_attribution(df, models_path):
    models_iterable = list(
        set(
            df.apply(
                lambda row: (row["selected_model_ttype"], row["selected_model_gene"]),
                axis=1,
            )
        )
    )
...
@cli.command()
@click.option(
    "--input", type=click.Path(), help="table of results after attribution run"
)
