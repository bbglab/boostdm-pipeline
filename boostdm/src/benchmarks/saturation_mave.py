...
mave_code = {
    "TP53_Kato": {"scores": ["tp53_score"], "aachange": "aachange"},
    "TP53_Boettcher": {"scores": ["FUNC_SCORE"], "aachange": "aachange"},
    "TP53_Ursu": {"scores": ["HotellingT2"], "aachange": "Variant"},
    "TP53_Kotler": {"scores": ["RFS_H1299"], "aachange": "aachange"},
    "TP53_Giacomelli": {"scores": ["FUNC_SCORE"], "aachange": "aachange"},
    "Ras_Bandaru": {"scores": ["FUNC_SCORE"], "aachange": "aachange"},
    "DNMT3A_Lue": {
        "scores": ["sgRNA_score_d9_citrine_positive"],
        "aachange": "aachange",
    },
    "PTEN_Matreyek": {"scores": ["score"], "aachange": "aachange"},
    "PTEN_Mighell": {"scores": ["Cum_score"], "aachange": "aachange"},
    "KRAS_Ursu": {"scores": ["HotellingT2"], "aachange": "Variant"},
}
...
def cli(input):
    ...
    for fn in glob.glob(
        os.path.join(
            basepath,
            f"saturation/prediction/{gene}.model.*.features.*.prediction.tsv.gz",
        )
    ):
    ...
    for gene, ttype_model, _ in tqdm.tqdm(pool):
        saturation_annotation_fn = os.path.join(
            basepath,
            "saturation",
            "annotation",
            f"{gene}.{ttype_model}.annotated.tsv.gz",
        )
        ...
        if gene == "TP53":
            for assay in tp53_assays:
                ...
                sat_annot_df = pd.merge(
                    sat_annot_df,
                    annot_df[[aachange, score]],
                    how="left",
                    left_on=["aachange"],
                    right_on=[aachange],
                    suffixes=("", "_y"),
                ...
                sat_annot_df.drop(
                    sat_annot_df.filter(regex="_y$").columns, axis=1, inplace=True
                )

            sat_annot_df.to_csv(
                f"{gene}.{ttype_model}.saturation.mave.tsv", sep="\t", index=False
            )
