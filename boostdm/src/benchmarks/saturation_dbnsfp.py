...
def cli(input):
    ...
    for gene, ttype_model, _ in tqdm.tqdm(pool):
        ...
        annot_df["chr"], annot_df["pos"] = zip(
            *annot_df["Location"].apply(lambda r: r.split(":"))
        )
        ...
        sat_annot_df = pd.merge(
            sat_df, annot_df, on=["chr", "pos", "alt"], suffixes=("", "_y")
        )
        sat_annot_df.drop(
        ...
        sat_annot_df.to_csv(
            f"{gene}.{ttype_model}.saturation.dbNSFP.tsv", sep="\t", index=False
        )
