...
def cli():
    for fn in tqdm.tqdm(
        glob.glob(
            os.path.join(
                os.environ["OUTPUT"], "benchmarks", "cv_tables", "*.*.50.iter.tsv"
            )
        )
    ):
        ...
        saturation_mave_fn = os.path.join(
            os.environ["OUTPUT"],
            "benchmarks",
            "saturation_mave",
            f"{gene}.{ttype}.saturation.mave.tsv",
        )
        if os.path.isfile(saturation_mave_fn):
            ...
            if gene == "TP53":
                cv_annot = pd.merge(
                    cv_df,
                    sat_annot[["chr", "pos", "alt"] + tp53_assays],
                    on=["chr", "pos", "alt"],
                    how="left",
                )
            ...
            if gene == "PTEN":
                cv_annot = pd.merge(
                    cv_df,
                    sat_annot[["chr", "pos", "alt"] + pten_assays],
                    on=["chr", "pos", "alt"],
                    how="left",
                )
            ...
            if gene == "KRAS":
                cv_annot = pd.merge(
                    cv_df,
                    sat_annot[["chr", "pos", "alt"] + ras_assays],
                    on=["chr", "pos", "alt"],
                    how="left",
                )
            ...
            if gene in ["HRAS", "NRAS"]:
                cv_annot = pd.merge(
                    cv_df,
                    sat_annot[["chr", "pos", "alt"] + ["Ras_Bandaru"]],
                    on=["chr", "pos", "alt"],
                    how="left",
                )
            ...
            if gene == "DNMT3A":
                cv_annot = pd.merge(
                    cv_df,
                    sat_annot[["chr", "pos", "alt"] + dnmt3a_assays],
                    on=["chr", "pos", "alt"],
                    how="left",
                )
        ...
        sat_annot_fn = os.path.join(
            os.environ["OUTPUT"],
            "benchmarks",
            "saturation_dbNSFP",
            f"{gene}.{ttype}.saturation.dbNSFP.tsv",
        )
        ...
        cv_annot = pd.merge(
            cv_annot,
            sat_annot[["chr", "pos", "alt"] + all_dbnsfp_scores + ["aachange"]],
            on=["chr", "pos", "alt"],
            how="left",
        )
