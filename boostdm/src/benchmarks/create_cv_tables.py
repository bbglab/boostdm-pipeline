...
cvsplit_folder = os.path.join(
    basepath, "splitcv_meta"
)  # cross-validation split folders
models_folder = os.path.join(basepath, "training_meta")  # models folder
modeleval_folder = os.path.join(
    basepath, "evaluation"
)  # cross-validation scores after training
...
def get_cv_prediction(gene, ttype, exclude_train=False):
    ...
    for i, split in enumerate(splitcv):
        ...
        if exclude_train:
            ...
            muts = test_features[
                ~test_features.apply(
                    lambda x: (x["pos"], x["ref"], x["alt"]), axis=1
                ).isin(
                    train_features.apply(
                        lambda x: (x["pos"], x["ref"], x["alt"]), axis=1
                    )
                )
            ]
        ...
        features = muts[
            [c for c in muts.columns if c not in ["chr", "pos", "ref", "alt"]]
        ]
