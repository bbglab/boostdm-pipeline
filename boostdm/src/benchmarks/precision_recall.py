...
palette_dnNSFP = {
    "AlphaMissense_score": "#FFAD84",
    "EVE_score": "#FFC47E",
    "ESM1b_score": "#FFE382",
    "CADD_raw": "#E26EE5",
    "FATHMM_score": "#6C22A6",
    "MetaLR_score": "#6962AD",
    "MetaRNN_score": "#A084E8",
    "MutationAssessor_score": "#EA8FEA",
    "PROVEAN_score": "#FFAACF",
    "Polyphen2_HDIV_score": "#C21292",
    "Polyphen2_HVAR_score": "#711DB0",
    "REVEL_score": "#5D3587",
    "SIFT4G_score": "#FED9ED",
    "SIFT_score": "#E7BCDE",
    "VEST4_score": "#BB9CC0",
    "phyloP100way_vertebrate": "#944E63",
    "phyloP17way_primate": "#B47B84",
    "phyloP470way_mammalian": "#CAA6A6",
}
...
def plot_prc(gene, ttype, score, ax=None, plot=True, **kwargs):
    cv_table = os.path.join(
        basepath,
        "benchmarks",
        "cv_tables_annotated",
        f"{gene}.{ttype}.50.iter.annotated.tsv",
    )
    ...
    if prop_drivers >= 0.5:
        ...
        if num_to_drop > 0:
            drop_indices = np.random.choice(
                df[df["driver"] == 1].index, num_to_drop, replace=False
            )
        ...
        if num_to_drop > 0:
            drop_indices = np.random.choice(
                df[df["driver"] == 0].index, num_to_drop, replace=False
            )
            df = df.drop(drop_indices)
    ...
    if positive > 20:
        if plot:
            plot_pr_curve(
                y, probs, ax, label=f"{score}: auPRC={auc_score:.2}", **kwargs
            )
...
def plot_score_family(gene, ttype, score_family):
    ...
    fig.savefig(
        f"{gene}.{ttype}.{score_family}.prauc.png", dpi=150, bbox_inches="tight"
    )
...
@click.command()
@click.option(
    "--plots", is_flag=True, show_default=True, default=False, help="Dump all PR plots"
)
def cli(plots):
    pool = []
    for fn in glob.glob(
        os.path.join(
            basepath, "benchmarks", "cv_tables_annotated", "*.*.50.iter.annotated.tsv"
        )
    ):
