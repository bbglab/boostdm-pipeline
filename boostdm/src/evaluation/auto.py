...
def safe_fscore(y_true, y_pred, beta=1.0):
...
def evaluate(model):
    ...
    labels = model["y_test"]
    predicts = [
        model.predict_proba(x)[:, 1] for model, x in zip(classifiers, covariates)
    ]
    ...
    # MCC score
    res["mcc"] = [
        matthews_corrcoef(lab >= 0.5, pred >= 0.5)
    ...
    # precision
    res["precision"] = [
        safe_precision(lab >= 0.5, pred >= 0.5) for lab, pred in zip(labels, predicts)
    ]
    # NPV
    res["npv"] = [
        safe_npv(lab >= 0.5, pred >= 0.5) for lab, pred in zip(labels, predicts)
    ]
    # recall
    res["recall"] = [
        safe_recall(lab >= 0.5, pred >= 0.5) for lab, pred in zip(labels, predicts)
    ]
    # fscore
    res["fscore100"] = [
        safe_fscore(lab >= 0.5, pred >= 0.5) for lab, pred in zip(labels, predicts)
    ]
    # fscore: beta=0.5
    res["fscore50"] = [
        safe_fscore(lab >= 0.5, pred >= 0.5, beta=0.5)
        for lab, pred in zip(labels, predicts)
    ]
    # accuracy
    res["accuracy"] = [
        safe_accuracy(lab >= 0.5, pred >= 0.5) for lab, pred in zip(labels, predicts)
    ]
    # test dataset balance
    res["balance"] = [0.5 - lab.mean() for lab, pred in zip(labels, predicts)]
    # calibration
    res["calibration"] = [
        (pred.mean() - lab.mean()) / lab.mean() for lab, pred in zip(labels, predicts)
    ]
    # test size
...
@click.option(
    ...
    help="File corresponding to models for oncotree instance",
)
