from src.globals import COHORTS_PATH, ONCOTREE_PATH
...
class Oncotree:
    ...
    def to_dict(self):
        d = {}
        children_parent_pairs = list(
            zip(self.tree_table["ID"], self.tree_table["PARENT"])
        )
    ...
    def get_cohorts(self, ttype):
        ...
        cohorts = self.stats_cohorts[self.stats_cohorts["CANCER_TYPE"] == ttype][
            "COHORT"
        ].tolist()
        ...
        if (
            ttype not in self.tree
        ):  # ttype is a leaf, then return cohorts gathered from self.stats_cohorts
            return cohorts
    ...
    def fetch_parent_cohort(self, cohort):
        ...
        :return: name of the parent
        """
        parent = self.stats_cohorts[self.stats_cohorts["COHORT"] == cohort][
            "CANCER_TYPE"
        ].unique()
        if len(parent) > 0:
            return parent[0]
