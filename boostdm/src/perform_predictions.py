...
from src.globals import COLUMNS_TRAINING, COLUMNS_OUTPUT, COLUMNS_SHAP, SYSTEMATIC_BIAS
from src.evaluation.data import Hierarchy
...
def _predict(mutations, models_folder, evaluations_folder, high_quality_only=True):
    ...
    for name, df in mutations.groupby(["selected_model_gene", "selected_model_ttype"]):
        ...
        path_eval = os.path.join(
            evaluations_folder, f"{ttype}", f"{gene}.eval.pickle.gz"
        )
        with gzip.open(path_eval, "rb") as g:
            evaluation = pickle.load(g)

        consensus_func = weighted_consensus(model, evaluation)
        mutations.loc[df.index, ["boostDM_score"]] = consensus_func(
            df[COLUMNS_TRAINING]
        )
    ...
    mutations.loc[:, "boostDM_class"] = mutations["boostDM_score"].apply(
        lambda x: x >= 0.5
    )
    return mutations
...
def predict(
    ...
    path_model = os.path.join(
        models_folder, f"{selected_model_ttype}", f"{gene}.models.pickle.gz"
    )
    with gzip.open(path_model, "rb") as f:
        model = pickle.load(f)

    path_eval = os.path.join(
        evaluations_folder, f"{selected_model_ttype}", f"{gene}.eval.pickle.gz"
    )
    with gzip.open(path_eval, "rb") as g:
        evaluation = pickle.load(g)
    ...
    mutations.loc[mutations.index, ["boostDM_score"]] = consensus_func(
        mutations[COLUMNS_TRAINING]
    )
...
@click.option(
    ...
    help="File with the annotated mutations",
    required=True,
)
@click.option("--gene", type=str, help="gene symbol", required=True)
@click.option(
    "--tumor-type", type=str, help="Tumor type of the input mutations", default=None
)
@click.option(
    "--models-folder",
    type=click.Path(exists=True),
    help="Path to the folder where the models are",
    required=True,
)
@click.option(
    "--evaluations-folder",
    type=click.Path(exists=True),
    help="Path to the folder where the evaluations are",
    required=True,
)
@click.option(
    "--model-selection",
    type=str,
    help="Either the path to the model selection dict, or the gene to be used",
    required=True,
)
@click.option(
    "--high-quality-only",
    ...
    show_default=True,
    default=False,
    help="Prediction only if matching models are high-quality",
)
def cli(
    muts,
    gene,
    ...
    if os.path.exists(model_selection):
        ...
        df, selected_model_ttype = predict(
            df_mutations,
            gene=gene,
            ttype=tumor_type,
        ...
        df, _ = predict_with_model(
            df_mutations,
            (tumor_type, model_selection),
            models_folder=models_folder,
            evaluations_folder=evaluations_folder,
        )
    ...
    output_file = (
        f"{gene}.model.{selected_model_ttype}.features.{tumor_type}.prediction.tsv.gz"
    )

    df[output_cols].to_csv(output_file, index=False, compression="gzip", sep="\t")
