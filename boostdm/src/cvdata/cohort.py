...
def split_balanced(x_data, y_data, test_size=0.3):
    ...
    x_data_sub = x_data.loc[one_index + zero_index, :].copy()
    y_data_sub = y_data.loc[one_index + zero_index].copy()

    # the random state should be fixed prior to this call
    x_train, x_test, y_train, y_test = train_test_split(
        x_data_sub, y_data_sub, test_size=test_size
    )
...
def get_cv_sets_balanced(x_data, y_data, n, size):
    ...
    for _ in range(n):
        x_train, x_test, y_train, y_test = split_balanced(
            x_data, y_data, test_size=size
        )
