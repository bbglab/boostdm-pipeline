...
from src.globals import MANE_TRANSCRIPTS_FILE, MNVS_FILE, COHORTS_PATH, DRIVERS_PATH
...
from src.passengers import retrieve_exons, randomize
...
def features(df, cohort, clustl_group_path, hotmaps_group_path, smregions_group_path):
    ...
    hotmaps_ttype_data = hotmaps_global_data[
        hotmaps_global_data["CANCER_TYPE"] == ttype
    ]
    ...
    smregions_ttype_data = smregions_global_data[
        smregions_global_data["CANCER_TYPE"] == ttype
    ]
...
def retrieve_transcript():
    ...
    mane_transcript_df = pd.read_csv(
        MANE_TRANSCRIPTS_FILE,
        sep="\t",
        header=None,
        compression="gzip",
        low_memory=False,
...
def intersect_region_mutations(cds, pos):
    ...
    drivers = pos_bed.intersect(cds_bed, wao=True).to_dataframe(
        usecols=[0, 2, 3, 4, 5], names=["chr", "pos", "ref", "alt", "gene"]
    )
...
def load_mutations(dndscv_annotated, drivers, cohort):
    df = pd.read_csv(
        dndscv_annotated, sep="\t", float_precision=None
    )  # os.path.join(dndscv_path, f'{cohort}.annotmuts.gz')
    ...
    df["cohort"] = cohort
    df["tuple"] = df.apply(
        lambda x: (x["gene"], x["sampleID"], x["cohort"], x["pos"]), axis=1
    )
...
def build_negative_set(counts_per_gene, mutrate, n_splits=50):
    ...
    for gene in tqdm.tqdm(counts_per_gene):
        ...
        for chr_, mut in randomize(
            mutrate, chrom, cds, segments, n_splits * counts_per_gene[gene]
        ):
...
def build_table(
    cohort,
    dndscv_file,
    dndscv_annotated_file,
    mutrate_fn,
    clustl_group_file,
    hotmaps_group_file,
    smregions_group_file,
    xs_thresh=0.85,
    n_splits=50,
):
    ...
    # Add features
    df = features(
        df, cohort, clustl_group_file, hotmaps_group_file, smregions_group_file
    )
...
@click.option(
    ...
    help="Cohort dNdsCV out",
)
@click.option(
    "--dndscv-annotmuts-path",
    "dnds_muts_path",
    type=click.Path(exists=True),
    help="Cohort dNdsCV annotmuts out",
)
@click.option(
    "--mutrate-path",
    "mutrate_path",
    type=click.Path(exists=True),
    help="Cohort mutrate out",
)
@click.option(
    "--clustl-group-path",
    "clustl_group_path",
    type=click.Path(exists=True),
    help="Combined OncodriveCLUSTL out",
)
@click.option(
    "--hotmaps-group-path",
    "hotmaps_group_path",
    type=click.Path(exists=True),
    help="Combined HotMAPS out",
)
@click.option(
    "--smregions-group-path",
    "smregions_group_path",
    type=click.Path(exists=True),
    help="Combined smregions out",
)
